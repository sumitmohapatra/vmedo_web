...
import { Observable, OperatorFunction, Subject, debounceTime, distinctUntilChanged, filter, switchMap } from 'rxjs';
   ...
   return new Promise((_res: any, _rej: any) => {
     this.common.api.GetCorpProfile(this.common.userInfo.userID).then((res: any) => {
       if(res.objret){
         ...
         this.txtPhone = res.objret.emp_mobile;
         this.txtEmail = res.objret.emp_email;
         this.orgName = res.objret.emp_organisation;
         this.totcredits = res.objcredit.credit_assigined;
         this.creditsused = res.objcredit.credit_used;
         this.credavail = res.objcredit.credit_available;
         
         this.txtGender = res.objret.emp_gender === null ? '' : res.objret.emp_gender;
         this.txtempbg = res.objret.emp_bloodgroup === null ? '' : res.objret.emp_bloodgroup;
         this.chkBloodDonor = res.objret.isDoner === 'NO' ? false : true;

         if(this.common.IsFile(res.objret.emp_profilepic)){
           const album = {
             src: res.objret.emp_profilepic
           };
           this.profileImage.push(album);
         }
         if(res.objret.emp_dob.toString() !== '0001-01-01T00:00:00')
           this.txtDOB = this.common.StringToDate(res.objret.emp_dob.toString().split('T')[0]);
...
//view emergency card
 ...
 IsValidProfile = () => {
   ...
   if(this.txtempbg === undefined && this.txtempbg === '') {
    this.lblempbg = "Please enter a valid gender !";
    return false;
  }

    if(this.txtempbg === undefined && this.txtempbg === '') {
     this.lblempbg = "Please enter a valid blood group !";
     return false;
   }
 ...
 OnClickCorpUpdate = () => {
   if(this.IsValidProfile()){
     ...
     this.app.ShowLoader();
     this.common.api.UpdateCorpProfile(userInfo).then(() => {
       this.app.HideLoader();
       this.app.ShowSuccess(`Employee profile updated successfully`);
     }).catch((rej) => {
       this.app.HideLoader();
       this.app.ShowError(`${rej.message}`);
     });
...
OnClickfourNext(txtCardNumber): void {
 ...
 OnClickCorpSubmit = () => {
   if(this.IsValidInsuranceDetails()){
     ...
     if(this.IsEmergencyEntityEdit === true){
       this.common.api.UpdateCorpEmergencyCard(cardInfo, this.common.userInfo.userID).then((res: any) => {
         this.app.HideLoader();
         this.CloseCreateEditIDModal();
       ...
       this.common.api.AddCorpEmergencyCard(cardInfo, this.common.userInfo.userID).then((res: any) => {
         this.app.HideLoader();
         this.CloseCreateIDModal();
 ...
 searchLocation: OperatorFunction<string, readonly string[]> = (text$: Observable<string>) =>{
   return text$.pipe(
     ...
     distinctUntilChanged(),
     switchMap( (searchText) =>  this.common.api.GetLocations(searchText) ),
     // map(term => (term === '' ? this.searchDonorLocations
     //     : this.searchDonorLocations.filter((v: any) => v.toLowerCase().indexOf(term.toLowerCase()) > -1)).slice(0, 30))
   )
 };
  ...
  this.http.get('https://apitest.vmedo.com/api/vadmin/SearchALLEmergencyIDPagination?Svalue=' + cardNumber, { headers }).subscribe(
    data => {
      if (data['statusCode'] === 200) {
        ...
        this.router.navigate(['/dashboard/downloadcorpemid']);
...
@ViewChild('newfeature') newfeatureIDModal: any;
newfeatureIDModalRef: any;

OpennewfeatureModal = () => {
  this.newfeatureIDModalRef = this.common.modal.OpenModal(this.newfeatureIDModal);
  this.newfeatureIDModalRef.result.finally(() => {
  });
};
