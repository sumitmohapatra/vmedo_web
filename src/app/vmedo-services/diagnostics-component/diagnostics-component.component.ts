...
@Component({
  selector: 'app-diagnostics-component',
  templateUrl: './diagnostics-component.component.html',
  styleUrls: ['./diagnostics-component.component.css'],
})
export class DiagnosticsComponentComponent {
  ...
  selectedDiagnostics: string | null = null; // Track the selected specialty

  togglePopup2(diagnostics: string) {
    this.selectedDiagnostics = diagnostics; // Set the selected specialty
  }

  clearPopup() {
    this.selectedDiagnostics = null; // Clear the selected specialty when closing the popup
  }

  // Toggle Urgentcare at home
  selectedDiagnosticsDesktop: string | null = null; // Track the selected specialty

  togglePopup2Desktop(diagnosticsDesktop: string) {
    this.selectedDiagnosticsDesktop = diagnosticsDesktop; // Set the selected specialty
  }

  clearPopupDesktop() {
    this.selectedDiagnosticsDesktop = null; // Clear the selected specialty when closing the popup
  }
  ...
  ngOnInit(): void {
    ...
    this.titleService.setTitle(
      'Diagnostics at home | ECG, PFT, Blood tests at home'
    );
